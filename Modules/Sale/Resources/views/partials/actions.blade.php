<div class="btn-group dropleft">
    ...
    <div class="dropdown-menu">
        ...
        <a target="_blank" href="{{ route('sales.pos.receipt', $data->id) }}" class="dropdown-item">
            <i class="bi bi-file-earmark-pdf mr-2 text-success" style="line-height: 1;"></i> Receipt Invoice
        </a>
        ...
        @can('access_sale_payments')
            @if ($data->due_amount > 0)
                @if ($data->payment_method == 'Midtrans' && $data->snap_token)
                    <button type="button" class="dropdown-item retry-midtrans-payment"
                        data-token="{{ $data->snap_token }}">
                        <i class="bi bi-arrow-clockwise mr-2 text-info" style="line-height: 1;"></i> Coba Bayar Ulang
                    </button>
                @else
                    {{-- Tombol Add Payment standar untuk metode lain --}}
